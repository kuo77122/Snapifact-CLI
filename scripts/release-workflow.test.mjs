...
test('Production finalization is success-gated, contents-write-only, and identity-only', () => {
  ...
  assert.match(finalizer, /RELEASE_ID: \$\{\{ needs\.publish\.outputs\.release_id \}\}/)
  assert.match(finalizer, /\[\[ "\$RELEASE_ID" =~ \^\[0-9\]\+\$ \]\]/)
  assert.match(finalizer, /gh api "repos\/\$GITHUB_REPOSITORY\/releases\/\$RELEASE_ID"/)
  assert.match(finalizer, /\.id \| type == "number"/)
  assert.match(finalizer, /\.draft == true/)
  assert.match(finalizer, /\.tag_name == \$version/)
  ...
  assert.match(finalizer, /gh api --method PATCH "repos\/\$GITHUB_REPOSITORY\/releases\/\$RELEASE_ID" -F draft=false/)
  assert.doesNotMatch(finalizer, /releases\/tags\/\$RELEASE_VERSION|gh release edit/)
})

test('publish selects exactly one numeric draft Release ID after creation', () => {
  assert.match(publish, /release_id: \$\{\{ steps\.release_identity\.outputs\.release_id \}\}/)
  assert.match(publish, /id: release_identity/)
  assert.match(publish, /gh api --paginate "repos\/\$GITHUB_REPOSITORY\/releases\?per_page=100"/)
  assert.match(publish, /\.draft == true and \.tag_name == \$version/)
  assert.match(publish, /jq 'length'/)
  assert.match(publish, /\[\[ "\$release_id" =~ \^\[0-9\]\+\$ \]\]/)
  assert.match(publish, /printf 'release_id=%s\\n' "\$release_id" >> "\$GITHUB_OUTPUT"/)
  assert.ok(publish.indexOf('name: Create one draft release') < publish.indexOf('id: release_identity'))
  assert.ok(publish.indexOf('id: release_identity') < publish.indexOf('id: draft_assets'))
})
